...
def recommand(movie):
    movie_index = movies[movies['title']==movie].index[0]   #fetching movie index from dataframe
    vector = similarity[movie_index]
    movies_list = sorted(list(enumerate(vector)), reverse=True, key=lambda x: x[1] )
